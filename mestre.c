...
typedef struct {
    ...
    int numCidades; // Adiciona um contador para o número de cidades cadastradas
} Estado;

typedef struct {
    char nome[50];
    Estado estados[MAX_ESTADOS];
    int numEstados; // Adiciona um contador para o número de estados cadastrados
} Pais;

// Declaração antecipada da função exibirDados
void exibirDados(Pais pais);

void cadastrarCidades(Estado *estado) {
    estado->numCidades = 0; // Inicializa o contador de cidades
    for (int j = 0; j < MAX_CIDADES; j++) {
        ...
        estado->numCidades++; // Incrementa o contador de cidades cadastradas
    }
}
...
void exibirDados(Pais pais) {
    printf("\n--- Dados do País: %s ---\n", pais.nome);
    for (int i = 0; i < pais.numEstados; i++) { // Usa o contador de estados
        Estado estado = pais.estados[i];
        if (estado.nome[0] != '\0') { // Verifica se o estado foi cadastrado
            printf("\nEstado: %s\n", estado.nome);
            for (int j = 0; j < estado.numCidades; j++) { // Usa o contador de cidades
                Cidade cidade = estado.cidades[j];
        ...
        switch (opcao) {
            case 1:
                if (numPaises < MAX_PAIS) {
                    Pais *paisAtual = &paises[numPaises]; // Aponta para o país atual
                    printf("\n--- Cadastro do País ---\n");
                    printf("Nome do país: ");
                    scanf("%s", paisAtual->nome);

                    for (int j = 0; j < MAX_ESTADOS; j++) {
                        Estado *estadoAtual = &paisAtual->estados[j]; // Aponta para o estado atual
                        printf("\nCadastro do Estado %d:\n", j + 1);
                        printf("Nome do estado: ");
                        scanf("%s", estadoAtual->nome);
                        cadastrarCidades(estadoAtual); 
                        paisAtual->numEstados++; // Incrementa o contador de estados cadastrados
                    }
            ...
            case 3:
                if (numPaises > 0) {
                    ...
                    for (int i = 0; i < paisSelecionado.numEstados; i++) { 
                        Estado estado = paisSelecionado.estados[i];
                    ...
                    // Busca as cidades escolhidas usando os índices corretos
                    for(int i = 0; i < paisSelecionado.numEstados; i++){
                        for(int j = 0; j < paisSelecionado.estados[i].numCidades; j++){
